  ...
  return (
    <motion.div layout>
      <Link href={`/projects/${project.slug}`} className="group block h-full">
        <article className="h-full rounded-xl border border-[var(--border)] bg-[var(--card)] p-6 flex flex-col gap-4 transition-all duration-300 hover:border-accent hover:shadow-lg hover:shadow-accent/5 hover:-translate-y-0.5">
          <div className="flex items-start justify-between gap-3">
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full capitalize ${typeColors[project.type]}`}>
              {project.type.replace("-", " ")}
            </span>
            <span className="text-xs text-[var(--fg-muted)] font-mono">{project.year}</span>
          </div>
